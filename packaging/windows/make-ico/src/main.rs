...
//! sources.
//!
//! **One drawing, where segler's copy of this tool reads three.** That
//! application claims two file kinds and needs a `uap:Logo` for each; Duckling
//! owns no format, so the four assets below all come from `duckling.svg` and
//! there are no file-type logos to write. `AppxManifest.xml.in` declares the
//! fifteen types Duckling *reads* and gives none of them a logo, which is the
//! same statement in the other file: those types are other people's, and their
//! icons should stay other people's.
//!
...
/// upscale of it. `duckling.ico` carries nine sizes for exactly this reason and
...
/// The four images the manifest names, and nothing else.
///
...
/// application list entry - is drawn at the size it is given and wants the
/// whole of it, which is also what `duckling.ico` does at every size it holds.
/// Only a look at real tiles settles the two thirds, and that look was taken on
/// slipcase-desktop and settled them there.
///
/// Four where segler's copy has six: the two missing ones are that
/// application's file-type logos, and Duckling declares no logo for any type it
/// reads.
const ASSETS: &[Asset] = &[
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let svg = args
        .next()
        .map_or_else(|| here.join("../../linux/icons/duckling.svg"), PathBuf::from);
    ...
    let path = destination.join("duckling.ico");
