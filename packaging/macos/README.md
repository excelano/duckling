...
build, check and photograph it. `DESIGN.md` §8 has the reasoning.
    ...
    shots.sh              the listing's two frames as recipes: which documents and which state
    screenshot.sh         the driver under it, and take-shots.sh the set-taking over it;
                          both generated by ship, so edit the template and not the copy
